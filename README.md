# zFeder ESO Addons

Small Elder Scrolls Online addons and tools for cleaner UI, practical utility, setup management, arena practice, and PC gameplay workflows.

This repository is prepared for GitHub Pages. The static site is in the repository root:

- `index.html`
- `styles.css`
- `assets/Logo_eso_addons_transparent.png`
- `assets/zfeder-icon-transparent.png`

## Current Projects

- zFeder Gamepad Camera Boost
- HUD, utility, and arena research

## Support

Support is completely optional and helps with testing, maintenance, and future development.

Support page coming soon.

## Disclaimer

zFeder is an independent fan project. The Elder Scrolls Online and related marks are trademarks of their respective owners.
